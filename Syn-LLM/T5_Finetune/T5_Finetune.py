...
tokenizer = T5Tokenizer.from_pretrained("D:\Flan_T5_Large")
t5model = T5ForConditionalGeneration.from_pretrained("D:\Flan_T5_Large")
...
# train_dataset = TripletDataset(file_path, tokenizer)
file_paths = [
...
SDRmodel = SemanticDynamicRoutingModule(hidden_size=1024, num_attention_heads=4).to(device)
...
for epoch in range(num_epochs):
    ...
    with open("T5_Based_Full.txt", "a", encoding="utf-8") as f:
        print(f"Epoch {epoch + 1} Average Loss: {avg_loss:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}", file=f)


# 保存微调后的模型
output_dir = "D:/Project/SAGF/Model_Path/AdaLoRA_T5_Large_Full"
